...
class InterpretableSchNet(SchNet):
    """
    ...
    def __init__(
        ...
        super(InterpretableSchNet, self).__init__(
            embedding_layer,
            interaction_blocks,
            rbf_layer,
            output_network,
        ...
        if edge_reg:
            ...
            self.register_parameter(
                "edge_params_all",
                torch.nn.Parameter(edge_modulation, requires_grad=edge_reg),
            )
        self.edge_reg = edge_reg
    ...
    def forward(self, data: AtomicData) -> AtomicData:
        ...
        ### ___ Addition for RBF filtering ___
        senders = data.atom_types[edge_index[0]]
        receivers = data.atom_types[edge_index[1]]
        ...
        # Radial filters are clamped from 0 to 1
        rbf_filters = torch.clamp(
            self.radial_filters[senders, receivers], min=0.0, max=1.0
        )

        rbf_expansion = self.rbf_layer(distances) * rbf_filters
        ### ___ End of addition ___
