...
def radius(
    ...
    if (cell is not None) and (pbc is not None):
        ...
        apply_pbc = False
        cell = torch.empty(0, device=pos.device)
        pbc = torch.empty(0, device=pos.device)
...
def setup_context(ctx, inputs, output):
    ...
    )  # FIXME add computation if not done already
...
def backward(ctx, grads):
    ...
    if ctx.needs_input_grad[0]:
        ...
        unit_vec = torch.div(displacements, distances)
        expanded_pos_grad = (
