...
def grad_x_grad_edge_weight_fused_cfconv_kernel(
    ...
    grad_edge_weight_ptr,  # [num_edges] - OUTPUT (FP32 or FP16)
    ...
    GRAD_EDGE_WEIGHT_FP16: tl.constexpr,  # Whether to output FP16
    ...
    Computes:
        Computes: grad_x[src] = sum_{e: src[e]=src} grad_edge_weight[e] * grad_output[dst[e]] * filters[e] * d_cutoff_dd[e]
    """
    ...
    for f_start in range(0, feature_dim, BLOCK_F):
        ...
        for e_csr in range(seg_start_src, seg_end_src):
            ...
            grad_edge_weight = tl.load(grad_edge_weight_ptr + edge_idx)
            if GRAD_EDGE_WEIGHT_FP16:
                grad_edge_weight = grad_edge_weight.to(tl.float32)

            acc += grad_edge_weight * filters * grad_output * C
...
def grad_x_grad_edge_weight_fused_cfconv(
    ...
    grad_edge_weight: torch.Tensor,
    src_perm: torch.Tensor,
    src_ptr: torch.Tensor,
    cutoff_upper: float,
    grad_edge_weight_dtype: torch.dtype = None,
) -> torch.Tensor:
    """
    ...
    Args:
        ...
        grad_edge_weight: Gradients w.r.t edge weights [num_edges]
        ...
        grad_edge_weight_dtype: Data type of grad_edge_weight (FP32 or FP16)
    ...
    grad_edge_weight_fp16 = grad_edge_weight_dtype == torch.float16
    ...
    wrap_triton(grad_x_grad_edge_weight_fused_cfconv_kernel)[grid](
        ...
        GRAD_EDGE_WEIGHT_FP16=grad_edge_weight_fp16,
    )
...
def backward_grad_x_grad_edge_weight_fused_cfconv(ctx, grad_grad_x):
    ...
    grad_grad_output = grad_filters = grad_edge_weight_out = (
    ...
    if ctx.needs_input_grad[2]:
        ...
        grad_edge_weight_out = (
            (grad_grad_x[edge_src] * grad_output[edge_dst] * filters).sum(dim=1)
            * d2C_dd2
            * grad_edge_weight
        )
    ...
    if ctx.needs_input_grad[5]:
        grad_grad_edge_weight = (
            grad_grad_x[edge_src] * grad_output[edge_dst] * filters
        ).sum(dim=1) * dC_dd
...
def grad_grad_out_grad_edge_weight_fused_cfconv_kernel(
    ...
    Computes:
        Computes: grad_grad_out[dst] = sum_{e: dst[e]=dst} grad_edge_weight[e] * x[src[e]] * filters[e] * d_cutoff_dd[e]
    ...
    for f_start in range(0, feature_dim, BLOCK_F):
        ...
        for e_csr in range(seg_start_dst, seg_end_dst):
            ...
            acc += grad_edge_weight * x * filters * C
...
def setup_context_grad_grad_out_grad_edge_weight_fused_cfconv(
    ...
    ctx.save_for_backward(
        x, filters, edge_weight, edge_src, edge_dst, grad_edge_weight
    )
...
def backward_grad_grad_out_grad_edge_weight_fused_cfconv(
    ...
    grad_x = grad_filters = grad_edge_weight = grad_grad_edge_weight = None
    ...
    if ctx.needs_input_grad[2]:
        d2C_dd2 = _torch_d2_cosine_cutoff_dd2(edge_weight, cutoff_upper)
        grad_edge_weight = (
            (grad_grad_grad_out[edge_dst] * x[edge_src] * filters).sum(dim=1)
            * d2C_dd2
            * grad_edge_weight
        )
    ...
    if ctx.needs_input_grad[5]:
        grad_grad_edge_weight = (
            grad_grad_grad_out[edge_dst] * x[edge_src] * filters
        ).sum(dim=1) * dC_dd
...
def grad_filters_grad_edge_weight_fused_cfconv_kernel(
    ...
    Computes:
        Computes: grad_filters[e] =  grad_edge_weight[e] * grad_output[dst[e]] * x[src[e]] * d_cutoff_dd[e]
...
def backward_grad_filters_grad_edge_weight_fused_cfconv(ctx, grad_grad_filters):
    ...
    grad_x = grad_grad_output = grad_edge_weight = grad_grad_edge_weight = None
    ...
    if ctx.needs_input_grad[2]:
        d2C_dd2 = _torch_d2_cosine_cutoff_dd2(edge_weight, cutoff_upper)
        grad_edge_weight = (
            (grad_grad_filters * grad_output[edge_dst] * x[edge_src]).sum(dim=1)
            * d2C_dd2
            * grad_edge_weight
        )
    ...
    if ctx.needs_input_grad[5]:
        grad_grad_edge_weight = (
            grad_grad_filters * grad_output[edge_dst] * x[edge_src]
        ).sum(dim=1) * dC_dd
...
def cpu_grad_filters_grad_edge_weight_fused_cfconv(
    ...
    dC_dd = _torch_d_cosine_cutoff_dd(edge_weight, cutoff_upper)
    grad_filters = (
        grad_edge_weight.unsqueeze(1) * grad_output[edge_dst] * x[edge_src]
    ) * dC_dd.unsqueeze(1)
...
def grad_edge_weight_grad_edge_weight_fused_cfconv_kernel(
    ...
    edge_src_ptr,  # [num_edges]
    edge_dst_ptr,  # [num_edges]
    grad_edge_weight_ptr,  # [num_edges, feature_dim] - OUTPUT (FP32 or FP16)
    # Output pointers
    grad_edge_weight_out_ptr,  # [num_edges]
    # Cutoff parameters
    cutoff_upper,
    # Sizes
    num_edges,
    ...
    for f_start in range(0, feature_dim, BLOCK_F):
        ...
        acc += tl.sum(grad_out * x * filters, axis=-1)

    tl.store(grad_edge_weight_out_ptr + edge_idx, grad_edge_weight * acc * C)
...
def grad_edge_weight_grad_edge_weight_fused_cfconv(
    ...
    Computes: grad_edge_weight_out[e] = grad_edge_weight[e] * (grad_output[dst[e]] * x[src[e]] * filters[e]).sum(axis=-1) * d2_cutoff_dd2[e]
    ...
    Args:
        ...
        edge_src: Source node indices [num_edges]
        edge_dst: Destination node indices [num_edges]
        grad_edge_weight: Gradients w.r.t edge representations [num_edges]
        cutoff_upper: Upper bound for cosine cutoff function
        grad_edge_weight_dtype: Data type of grad_edge_weight (FP32 or FP16)
    ...
    grad_edge_weight_out = torch.zeros(
        num_edges, device=x.device, dtype=torch.float32
    ).contiguous()

    if num_edges == 0:
        return grad_edge_weight_out
...
def backward_grad_edge_weight_grad_edge_weight_fused_cfconv(
    ...
    grad_x = grad_grad_output = grad_filters = grad_edge_weight_out = (
        grad_grad_edge_weight
    ) = None
    if ctx.needs_input_grad[0]:
        grad_x = torch.zeros_like(x)
        expanded = (
            grad_grad_edge_weight_out.unsqueeze(1)
            * grad_edge_weight.unsqueeze(1)
            * grad_output[edge_dst]
            * filters
            * d2C_dd2.unsqueeze(1)
    ...
    if ctx.needs_input_grad[3]:
        ...
        grad_edge_weight_out = (
            grad_grad_edge_weight_out
            * grad_edge_weight
            * (grad_output[edge_dst] * x[edge_src] * filters).sum(dim=1)
            * d3C_dd3
        )
...
def cpu_grad_edge_weight_grad_edge_weight_fused_cfconv(
    ...
    return grad_edge_weight_out  # FIXME: check dtype
