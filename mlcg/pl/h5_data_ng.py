...
from mlcg.datasets.cython.collater import CythonCollater
from mlcg.datasets.cython.transform import NoiseMapTransformer
from ..datasets.h5_dataset_ng import H5DatasetNG as H5Dataset
from ..datasets import H5PartitionDataLoader, H5MetasetDataLoader
...
class H5DataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        transform_noise_level: Optional[float] = None,
        transform_kbt: Optional[float] = None,
        ...
        if transform_noise_level is not None and transform_noise_level != 0.0:
            assert transform_kbt is not None
            transform = NoiseMapTransformer(
                transform_noise_level, transform_kbt
            )
            print(transform)
        else:
            transform = None
        self._collater_fn = CythonCollater(
            transform=transform,
