...
```bash
mlcg-train_h5 fit --config ./training.yaml
```
...
```bash
mlcg-combine_model --ckpt ./ckpt/last.ckpt --prior ./prior_tag_prior_model.pt --out model_with_prior.pt
```
...
```bash
mlcg-nvt_langevin --config ./1L2Y_sim_demo.yaml
```
