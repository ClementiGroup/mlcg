...
@pytest.mark.parametrize(
    ...
    [
        (ExpNormalBasis(IdentityCutoff(1, 5)), CosineCutoff(0, 5), UserWarning),
    ],
)
def test_cutoff_warning(basis, cutoff, expected_warning):
    with pytest.warns(expected_warning):
        StandardFlashSchNet(basis, cutoff, [128, 128])


@pytest.mark.parametrize(
    "basis, cutoff, expected_warning",
    [
        (ExpNormalBasis(IdentityCutoff(0, 5)), CosineCutoff(0, 5), None),
    ],
)
